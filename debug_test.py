...
def test_monte_carlo_comparison():
    ...
    for M in sample_sizes:
        ...
        expectation_original = propagator.expectation_pauli_sum(sampled_paulis, "000000")
        print(f"原始方法估计值: {expectation_original:.6f}")
        
        # 修正后方法
        sampled_paulis_nonzero, _, _, _ = propagator.monte_carlo_samples_nonzero(init_term, M)
        expectation_nonzero = propagator.expectation_pauli_sum(sampled_paulis_nonzero, "000000")
        print(f"修正后方法估计值: {expectation_nonzero:.6f}")
